...
def get_biggest_bar(bar_list):
    ...
    return bar_list_tmp[-1]

def get_smallest_bar(bar_list):
    bar_list_tmp = sort_by_cells_item(bar_list)
    return bar_list_tmp[0]
...
def get_closest_bar(bar_list, longitude, latitude):
    ...
    return min(bar_list, key=lambda bar: get_distance(bar['Cells']['geoData']['coordinates'][1],latitude,\
...
if __name__ == '__main__':
    ...
    print("\nBiggest bar in Moscow is: '%s', %s" % (get_name(biggest_bar), get_address(biggest_bar)))
    print("Smallest bar in Moscow is: '%s', %s" % (get_name(smallest_bar), get_address(smallest_bar)))
    ...
    print("Closest bar is : '%s', %s" % (get_name(closest_bar), get_address(closest_bar)))
